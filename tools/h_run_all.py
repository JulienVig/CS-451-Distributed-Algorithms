...
def main():
    ...
    # Finding server
    with open('../config_files/configs/perfect-links.config', 'r') as f:
    ...
    with open('../config_files/hosts', 'r') as f:
        for l in f.readlines():
            perfect_link_id = int(l.split(' ')[0])
            process = subprocess.Popen(["./run.sh", "--id", str(perfect_link_id), '--hosts', '../config_files/hosts',
                                        '--output', '../config_files/outputs/', '../config_files/configs/perfect-links.config'],
